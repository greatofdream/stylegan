...
class ConstantInput(Module):
    ...
    def execute(self, input):
        ...
        return out
class FusedUpsample(Module):# use filter
    ...
    def execute(self, input):
        weight = nn.pad(self.weight * self.multiplier, [1, 1, 1, 1])# pading the last two axis by 1 in each side
        weight = (
            weight[:, :, 1:, 1:]
            + weight[:, :, :-1, 1:]
            + weight[:, :, 1:, :-1]
...
class EqualLR:# 调节学习率
    ...
    def apply(module, name):
        ...
        module.register_pre_forward_hook(fn)# 在模块forward之前执行
